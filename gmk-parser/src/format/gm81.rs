...
pub fn check(exe: &mut io::Cursor<&mut [u8]>) -> io::Result<bool> {
    info!("Checking for standard GM8.1 format");
    ...
    if buf == [0xE8, 0x80, 0xF2, 0xDD, 0xFF, 0xC7, 0x45, 0xF0] {
        ...
        let gm81_magic: Option<u32> = match buf {
            [0x81, 0x7D, 0xEC] => {
                ...
                if exe.read_u8()? == 0x74 {
                    info!("GM8.1 magic check looks intact - value is 0x{:X}", magic);
                    Some(magic)
                } else {
                    info!("GM8.1 magic check's JE is patched out");
                    None
                }
            },
            b => {
                info!("GM8.1 magic check's CMP is patched out ({:?})", b);
                None
            },
        };
        ...
        let xor_method = match buf {
            [0x8B, 0x02, 0xC1, 0xE0, 0x10, 0x8B, 0x11, 0x81] => {
                info!("Found SUDALV re-encryption");
                XorMethod::Sudalv
            },
            _ => XorMethod::Normal,
        };
        ...
        match gm81_magic {
            Some(n) => {
                info!("Searching for GM8.1 magic number {} from position {}", n, header_start);
                let found_header = seek_value(exe, n)?.is_some();
                if !found_header {
                    info!("Didn't find GM81 magic value (0x{:X}) before EOF", n);
                    return Ok(false)
                }
            },
            None => {
...
pub fn check_lazy(exe: &mut io::Cursor<&mut [u8]>) -> io::Result<bool> {
    info!("Searching for default GM8.1 data header");
    exe.set_position(3800004);
    let found_header = seek_value(exe, 0xF7140067)?.is_some();
    if found_header {
        info!("Found default GM8.1 header at offset {}", exe.position());
...
pub fn decrypt(data: &mut io::Cursor<&mut [u8]>, xor_method: XorMethod) -> io::Result<()> {
    ...
    info!("Decrypting GM8.1 protection (hashkey: {}, seed1: {}, seed2: {})", hash_key, seed1, seed2);
