    ...
    if buf == [0x8B, 0x45, 0xF4, 0xE8, 0x2A, 0xBD, 0xFD, 0xFF] {
        ...
        let gm80_magic: Option<u32> = match exe.read_u8()? {
            0x3D => {
                ...
                if buf == [0x0F, 0x85, 0x18, 0x01, 0x00, 0x00] {
                    info!("GM8.0 magic check looks intact - value is {}", magic);
                    Some(magic)
                } else {
                    info!("GM8.0 magic check's JNZ is patched out");
                    None
                }
            },
            0x90 => {
                exe.seek(SeekFrom::Current(4))?;
                info!("GM8.0 magic check is patched out with NOP");
                None
            },
            i => {
                info!("Unknown instruction in place of magic CMP: {}", i);
                return Ok(false)
            },
        };
        ...
        let gm80_header_ver: Option<u32> = {
            ...
            if buf == [0x8B, 0xC6, 0xE8, 0x07, 0xBD, 0xFD, 0xFF] {
                match exe.read_u8()? {
                    0x3D => {
                        ...
                        if buf == [0x0F, 0x85, 0xF5, 0x00, 0x00, 0x00] {
                            info!("GM8.0 header version check looks intact - value is {}", magic);
                            Some(magic)
                        } else {
                    ...
                    0x90 => {
                        exe.seek(SeekFrom::Current(4))?;
                        info!("GM8.0 header version check is patched out with NOP");
                        None
                    },
                    i => {
                        info!("Unknown instruction in place of magic CMP: {}", i);
                ...
                info!("GM8.0 header version check appears patched out");
                None
            }
        };
        ...
        info!("Reading header from 0x{:X}", header_start);
            ...
            Some(n) => {
                loop {
                    let header1 = match exe.read_u32::<LE>() {
                        Ok(h) => h,
                        _ => {
                            info!("Passed end of stream looking for GM8.0 header");
                            return Ok(false)
                        },
                    };
                    if header1 == n {
        ...
        match gm80_header_ver {
            Some(n) => {
                ...
                if header2 != n {
                    info!("Failed to read GM8.0 header: expected version {}, got {}", n, header2);
                    return Ok(false)
                }
            },
            None => {
...
pub fn decrypt(data: &mut io::Cursor<&mut [u8]>) -> io::Result<()> {
    ...
    info!("Decrypting asset data... (size: {}, garbage1: {}, garbage2: {})", len, garbage1_size, garbage2_size);
