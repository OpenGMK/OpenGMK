...
pub struct CodeAction {
    ...
    /// Parameters to the DnD option. There can be up to 8.
    /// These can have various types and will be evaluated at runtime.
    pub parameters: Vec<CodeActionParam>,

    /// Instance this applies to - can be -1 (self), -2 (other) or a positive number (Object ID).
...
impl CodeAction {
    ...
    pub fn deserialize(reader: &mut io::Cursor<&[u8]>, options: &ParserOptions) -> io::Result<CodeAction> {
        if options.strict {
        ...
        // For some reason it always compiles 8 params in but doesn't use all of them,
        // so it has two different counters for the number present and the number actually used.
        // The number present should always be 8.
        ...
        // If you ever change this, at the very least, make sure arg_count is equal to type_count.
        // Right now I'm asserting that both are 8.
