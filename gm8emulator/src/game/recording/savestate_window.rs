    ...
    fn show_window(&mut self, info: &mut EmulatorContext) {
        info.frame
            ...
            .build(|| {
                ...
                let pos = Vec2::<f32>::from(info.frame.window_pos())
                    + Vec2::<f32>::from(info.frame.window_content_region_min())
                    - Vec2(8.0, 8.0);
                ...
                for i in 0..self.capacity {
                    ...
                    let button_hovered_color =
                        info.frame.push_style_color(StyleColor::ButtonHovered, [0.98, 0.59, 0.26, 1.0]);
                    let button_active_color =
                        info.frame.push_style_color(StyleColor::ButtonActive, [0.98, 0.53, 0.06, 1.0]);
                    ...
                    if info.savestate_exists(i) {
                        if info.frame.button_with_size_and_pos(&self.load_text[i], Vec2(60.0, 20.0), Vec2(75.0, y))
                            && *info.startup_successful
                        {
                            info.savestate_load(i);
                        }

                        if info.frame.button_with_size_and_pos(&self.select_text[i], Vec2(60.0, 20.0), Vec2(146.0, y))
