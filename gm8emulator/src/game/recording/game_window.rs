...
impl GameWindow {
    ...
    fn display_window(&mut self, info: &mut DisplayInformation) {
        ...
        info.frame.begin_window(
            ...
            Some(imgui::Vec2(w as f32 + (2.0 * info.win_border_size), h as f32 + info.win_border_size + info.win_frame_height)),
            false,
            false,
            None,
        );
        ...
        if !info.frame.window_collapsed() {
            ...
            if *info.setting_mouse_pos && info.frame.left_clicked() {
                ...
                *info.new_mouse_pos = Some((
                    -(x + info.win_border_size - mouse_x) as i32,
                    -(y + info.win_frame_height - mouse_y) as i32,
                ));
            }
